...
class AudioCaptcha(object):
    ...
    def create_wave_body(self, chars, with_beep, with_noise, mix_mode):
        voices = []
        inters = []
        for key in chars:
        ...
        if with_noise is False:
            for ii in range(len(bg)):
                bg[ii] = 128
        if with_beep is False:
            for ii in range(len(BEEP)):
                BEEP[ii] = 0
        ...
        for i, v in enumerate(voices):
            ...
            if mix_mode is 'add':
                bg[pos:end] = mix_wave_add(v, bg[pos:end])
            else:
    ...
    def create_wave_body_pairs(self, chars, mix_mode):
        voices = []
        inters = []
        for key in chars:
            voices.append(self._twist_pick(key))
            # v = random.randint(WAVE_SAMPLE_RATE, WAVE_SAMPLE_RATE * 3)
            v = random.randint(int(WAVE_SAMPLE_RATE/8), int(WAVE_SAMPLE_RATE/4))
            inters.append(v)
        ...
        wave_ns = BEEP + SILENCE + BEEP + SILENCE + BEEP + bg + END_BEEP
        ...
        wave_clean = BEEP + SILENCE + BEEP + SILENCE + BEEP + bg + END_BEEP

        return wave_clean, wave_ns


    def generate(self, chars, with_beep=True, with_noise=True, mix_mode='prod'):
        ...
        if not self._cache:
            self.load()
        body = self.create_wave_body(chars, with_beep, with_noise, mix_mode)
        return patch_wave_header(body)


    def generate_pairs(self, chars, mix_mode='prod'):
        """Generate audio CAPTCHA data. The return data is a bytearray.
        ...
        if not self._cache:
            self.load()
        body_clean, body_ns = self.create_wave_body_pairs(chars, mix_mode)
        # return patch_wave_header(body_clean), patch_wave_header(body_ns)
        return body_clean, body_ns
